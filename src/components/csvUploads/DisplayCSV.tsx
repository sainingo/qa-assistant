...
import { getCsvFiles, deleteCsvFile, syncCsvFile } from "./csv.resource";
           ...
           <>
             <tr className="bg-white border-b" key={item.id}>
               ...
               <td className="px-3 py-2 flex items-center gap-3">
                 <button onClick={() => handleCsvSync(item.eid_file_upload_metadata_id)} className={`${!synced[item.eid_file_upload_metadata_id]  ? "bg-blue-400 p-2 rounded-md cursor-pointer text-white" : "bg-gray-200 p-2 rounded-md text-black font-light opacity-50 cursor-not-allowed"}`}>
                   SYNC
                 </button>
                 <span onClick={() => handleVoidedCsv(item.eid_file_upload_metadata_id)} className="text-2xl text-red-900/90 cursor-pointer">
                   <MdDelete />
                 </span>
               </td>
             </tr>
